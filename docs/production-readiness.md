...
- Release Docker image verification and publishing both cover `linux/amd64` and `linux/arm64`. Docker push is bounded with a step timeout so GHCR or Buildx stalls cannot block Release publication indefinitely.
- The `Publish Docker Image` manual workflow is the recovery path for republishing or repairing existing GHCR image tags without rebuilding desktop Release assets.
...
- GHCR image tags are present only after release publish succeeds; the official Web preview image is published for `linux/amd64` and `linux/arm64`
