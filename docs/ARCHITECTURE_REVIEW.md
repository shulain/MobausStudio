...
| 状态对应版本 | `5d91e19`（本轮自审修正代码；本文档更新紧随其后） |
| 验证状态 | 前端/脚本：lint、typecheck、测试 1547/1547、覆盖率阈值、安全门禁 20 项、构建，均在本机通过。**Rust：本机无工具链，`cargo fmt` / `clippy` / `test` 一项未跑 —— S1、R1、S2 与新增的 `services/secure_file.rs` 全部未经编译验证** |
...
0. ~~写入非原子~~ —— 自审时发现 `write_secure` 原为 truncate + write，进程中途崩溃会留下被截断的凭证文件；而同仓库更不敏感的配置导出反而有原子写。已改为「同目录临时文件 → `rename` 替换」，具体保证：
   - 临时文件名用 UUID v4，并以 `create_new(true)` 创建 —— 保证是新文件，不复用已存在文件的宽松权限，目标为符号链接时直接失败（TOCTOU）；
   - Unix 下创建即 `0o600`，不存在「内容已落盘、权限未收紧」的窗口；
   - 写入后 `sync_all()` 再替换，确保内容在目标可见前已落盘；写入或落盘失败时删除本次创建的临时文件 —— 它可能已含部分明文凭证；
   - **持久性边界（已知且刻意不实现）**：`sync_all()` 只同步临时文件自身，而替换动作修改的是父目录项。Unix 下若要保证断电后最新一次写入仍存活，还需在替换后同步父目录，本实现未做。取舍理由：断电丢失最新 token 只需重新登录，而旧凭证被破坏才是真正损失 —— 后者由「不先删」与「替换失败保留旧文件」保证，与目录同步无关；
   - **不做「先删后改名」**：删除成功而替换失败会导致旧凭证永久丢失，与原子性保证相反。替换步骤直接委托 `std::fs::rename`，依赖标准库为各平台提供的替换语义，不对具体系统调用做假设（那是可能变化的实现细节）；平台不支持覆盖时返回错误，旧文件保持不变；
   - 替换步骤抽为可注入参数（`write_secure_with`），使「替换失败」分支能被测试真实驱动。此前用「目标是目录」间接触发的写法**捕获不到「先删后改名」缺陷** —— `remove_file` 对目录同样失败且错误被忽略。现在的测试针对真实旧凭证文件注入替换失败，断言旧内容完好且无临时文件残留。
...
**按数据语义区分处理**（自审时补充）：并非所有中毒都该「恢复后继续用」。

| 全局状态 | 中毒处理 | 理由 |
| -------- | -------- | ---- |
| `KIRO_CLIENT_REGISTRATION`、`KIRO_IDC_CLIENT_REGISTRATION` | `clear_poison()` + `into_inner()` 恢复复用 | 客户端注册信息可整体重写，半更新不产生错配 |
| `KIRO_SOCIAL_AUTH_STATE` | `clear_poison()` + `into_inner()` 后**置空** | 存的是 PKCE `code_verifier` 与 `state`，二者必须成对有效。panic 可能留下半更新组合，复用会用错配参数交换 token。置空后调用方得到「未找到授权状态」，重新发起授权即可 —— 比复用可疑数据稳妥 |

对应实现为 `lock_tolerant()` 与 `lock_kiro_auth_state()` 两个函数。

**`clear_poison()` 不可省略**：`into_inner()` 只取回数据，不改变锁的中毒状态，之后每次 `lock()` 仍返回 `Err`。对「置空」语义而言这是致命的 —— 发起授权时写入的 `Some(...)`，会在交换 token 取用前被再次清空，导致 Kiro 授权**永久失败直到重启应用**，正是本项要消除的失效模式。
