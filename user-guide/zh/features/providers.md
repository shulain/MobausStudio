...
如果你使用的是企业网关、代理服务或私有部署接口，可以添加自定义提供商：
...
- 使用第三方代理服务
- 使用私有部署的 OpenAI 兼容接口
- 使用其他未内置的 OpenAI 兼容服务

### 如何添加自定义提供商

1. 点击侧边栏的「🔌 提供商」图标
2. 点击页面右上角的「➕ 添加自定义提供商」按钮
3. 填写提供商信息：
   - **名称**：自定义名称（如"公司内部 AI"）
   - **图标**：选择一个 emoji 图标
   - **描述**：简短描述（支持中英文）
   - **API 端点**：服务地址（如 `https://your-endpoint/v1`）
   - **API Key**：根据服务要求填写（可选）
4. 添加至少一个模型：
   - **模型 ID**：模型标识符（如 `gpt-4o`）
   - **显示名称**：在界面中显示的名称
   - **最大输出 Token**：模型的最大输出长度
   - **上下文窗口**：模型支持的上下文长度
   - **能力**：勾选模型支持的功能（视觉、函数调用、流式输出）
5. 点击「测试连接」确认配置正确
6. 点击「保存」完成添加

> 💡 **提示**：可以添加多个自定义提供商，每个独立配置和管理。

### 管理自定义提供商

- **编辑**：点击自定义提供商卡片上的「编辑」按钮修改配置
- **删除**：点击「删除」按钮移除自定义提供商
- **连接/断开**：与内置提供商一样管理连接状态

> ⚠️ **注意**：删除自定义提供商后，使用该提供商模型的对话将无法继续。
...
### Google 模型配额查看 (v0.7.0 新增)

连接 Google 提供商后，可以在提供商卡片中查看各模型的配额使用情况：

1. 找到已连接的 Google 提供商卡片
2. 点击「📊 模型配额」展开配额面板
3. 查看各模型的剩余配额百分比
4. 配额耗尽的模型会显示红色警告

> 💡 **提示**：点击「刷新」按钮可以获取最新的配额信息。
