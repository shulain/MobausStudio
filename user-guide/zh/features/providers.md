...
### 方式四：自定义提供商（OpenAI 兼容）

如果你使用的是企业网关、代理服务或私有部署接口，可以使用「Custom」进行连接：

1. 在提供商列表选择「Custom」
2. 填写 API Endpoint（例如 `https://your-endpoint/v1`）
3. 根据服务要求选择是否填写 API Key
4. 点击「测试连接」确认可用后保存

> 💡 **提示**：建议先测试连接，再添加模型，能更快定位配置问题。
